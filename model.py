...
class Policy(nn.Module):
    ...
    def act(self, inputs, rnn_hxs, masks, threshold, min_g, phasic_g, device, flip_g, sigmoid_g, sigmoid_range, action_selection, deterministic=False):
        value, actor_features, rnn_hxs, x = self.base(inputs, rnn_hxs, masks)
        dist = self.dist(actor_features)
        dist_entropy = dist.entropy()
        g = get_g_entropy(device, dist_entropy, threshold, min_g, phasic_g, sigmoid_g, sigmoid_range, flip_g)
        if action_selection:
            dist = self.dist_entropy(actor_features*g)
